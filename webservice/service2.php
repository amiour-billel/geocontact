...
require('../plugins/rating/_drawrating.php');
...
if (!empty($_REQUEST['cat']) && ($_REQUEST['cat']!='')&& ($_REQUEST['cat']!='all')){
...
if(!empty($secteur_id)) $where.=" AND stores.secteur=".$secteur_id."";
if(!empty($categorie_id)) $where.=" AND stores.categorie=".$categorie_id."";
if(!empty($wilaya)) $where.=" AND stores.wilaya LIKE '%".$wilaya."%'";
if(!empty($evenement)) $where.=" AND stores.evenement = ".$evenement;
if(!empty($limitation)) $limit=" LIMIT 0 ,".$limitation." ";
if(!empty($keywodarry)){	
